...
def fanout_tasks(state: AgentWriterState):
    ...
    print(f"✅ END fanout_tasks ({len(sends)} tasks dispatched)")
    return sends
...
AgentWriter.add_edge("reducer_agent", END)
...
response = agent_writer_app.invoke(
    {
        "user_topic": "Open Source vs Proprietary LLMs"
    },
...
if final_blog:
    output_path = f"{response["user_topic"]}.md"
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(final_blog)
    print(f"✅ Saved final blog to {output_path}")
else:
    print(response)
